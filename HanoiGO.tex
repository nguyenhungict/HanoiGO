...
\begin{enumerate}
  \item \textbf{Bước 0 --- Filter feasibility (Tiền xử lý):} Trước khi đưa vào mô hình thuật toán, hệ thống loại bỏ các địa điểm đóng cửa vào ngày tương ứng với lịch trình của người dùng (sử dụng thông tin \texttt{open\_days}). Việc này đảm bảo kết quả gom cụm của K-Means không bị sai lệch bởi các điểm không thể tham quan.

  \item \textbf{Bước 1 --- Clustering:} Phân nhóm các địa điểm khả thi (đã qua Bước 0) theo khu vực địa lý
  ...
  \item \textbf{Bước 2 --- Routing (Time-Window Aware):} Trong mỗi cụm, sắp xếp
        thứ tự tham quan bằng Greedy Nearest Neighbor có tích hợp ràng buộc
        khung giờ. Hệ thống sử dụng \textbf{Goong Distance Matrix API} (hoặc
        OSRM) để lấy ma trận thời gian di chuyển thực tế giữa tất cả các cặp
        địa điểm trong cụm. Tại mỗi bước, thuật toán tính thời gian đến dự kiến:

        \[
          T_{arrival}(j) = T_{current} + \text{OSRM\_Duration}[i][j]
        \]

        Nếu điểm $j$ chưa mở cửa, thuật toán tính thời gian chờ
        $T_{wait} = \max(0,\; \text{open\_time\_start}_j - T_{arrival})$.
        Hàm chi phí tích hợp thời gian chờ được sử dụng để chọn điểm tiếp
        theo tối ưu:

        \[
          Cost(j) = \text{TravelTime}[i][j] + T_{wait}(j)
        \]

        Điểm $j$ bị bỏ qua nếu $T_{arrival}(j) > \text{open\_time\_end}_j
        - \text{visit\_duration}_j$ (đến nơi không còn đủ thời gian tham
        quan). Cách tiếp cận này ưu tiên điểm ``đến là vào được ngay'' thay
        vì chỉ chọn điểm gần nhất về khoảng cách.

  \item \textbf{Bước 3 --- Time-Window Constraints trong GNN:} Kiểm tra thuật toán với ràng buộc thời gian thật (giờ đóng cửa, giờ nghỉ trưa \texttt{has\_break}). Quá trình xử lý rẽ nhánh theo 3 cấp độ:
        \begin{itemize}
            \item \textbf{Cấp 1 (Reorder):} Thử tráo đổi ưu tiên thứ tự tham quan các điểm trong ngày nhằm né giờ nghỉ trưa hoặc kịp giờ đóng cửa.
            \item \textbf{Cấp 2 (Reassign):} Chuyển địa điểm bị cấn giờ qua lộ trình của ngày hôm sau (nếu có lịch).
            \item \textbf{Cấp 3 (Remove \& Notify):} Khi không còn cách xếp khả thi, thuật toán loại bỏ và thông báo đề xuất một địa điểm khác thay thế trong cùng khu vực khoảng cách gần.
        \end{itemize}
\end{enumerate}
...
\medskip
\textbf{Cấu hình OSRM cho xe máy Hà Nội:}

Thay vì sử dụng profile ô tô hoặc xe đạp mặc định, hệ thống sử dụng profile
\texttt{motorbike.lua} được tùy chỉnh cho đặc thù giao thông Hà Nội:

\begin{itemize}
  \item \textbf{Vận tốc:} Đặt mức trần 35~km/h cho đường chính
        (\texttt{primary}, \texttt{secondary}) và 20--25~km/h cho đường nhánh
        --- phản ánh thực tế tốc độ xe máy trong nội đô.
  \item \textbf{Intersection penalty:} Tăng \texttt{traffic\_light\_penalty}
        lên 18~giây để bù đắp cho chu kỳ đèn đỏ dài (60--100s) tại các nút
        giao lớn.
  \item \textbf{Ưu tiên ngõ ngách:} Loại bỏ hình phạt giảm tốc đối với
        \texttt{service=alley} để engine ưu tiên dẫn đường qua các ngõ nhỏ
        --- đặc trưng lối đi tắt hiệu quả của xe máy Hà Nội.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Trip Planner:} Frontend gửi danh sách địa điểm và số ngày →
        ...
        clustering (Haversine) → gọi OSRM Table API lấy ma trận khoảng cách
        thực tế → Greedy Nearest Neighbor có tích hợp hàm chi phí
        $Cost = Duration + T_{wait}$ → kiểm tra time-window constraints
        (Reorder / Reassign / Remove) → trả về lịch trình JSON + GeoJSON
        route để render trên map.
