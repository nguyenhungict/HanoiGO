...
\begin{enumerate}
    ...
    Trong số các ứng viên $j$ hợp lệ, thuật toán chọn điểm có $Cost(j)$ nhỏ nhất. Nếu chỉ dùng tổng thời gian thuần, hiện tượng ``tie-breaking'' sẽ xảy ra: ví dụ, các điểm cùng bị đẩy qua giờ nghỉ trưa đến 13:00 sẽ có chi phí bằng nhau, mất đi tính liên kết địa lý. Hệ thống thiết kế hàm phạt có trọng số:
    \begin{equation}
        Cost(j) = 2 \times \Delta T(current, j) + \Big(T_{wait}(j) \times 60\Big)
    \end{equation}
    Hệ số nhân $\times 2$ trên thời gian di chuyển đóng hai vai trò: (1) \textbf{phạt nặng di chuyển tích cực} hơn thời gian chờ thụ động --- ưu tiên chọn điểm gần hơn thay vì chờ điểm xa; (2) \textbf{phá vỡ thế cân bằng (tie-breaking)} khi nhiều ứng viên bị đẩy cùng đến 13:00 sau nghỉ trưa (lúc đó $T_{wait}$ bằng nhau, và $\Delta T \times 2$ sẽ phân biệt điểm gần -- xa). Bằng cách này, thuật toán đảm bảo lộ trình không zic-zắc và không có kết quả ngẫu nhiên khi thời gian bằng nhau.
\end{enumerate}
...
Trong quá trình gọi API, nếu gặp lỗi giới hạn tốc độ (\texttt{OVER\_RATE\_LIMIT}), hệ thống áp dụng cơ chế \textbf{Exponential Backoff} (thử lại tối đa 3 lần với độ trễ tăng dần 1s $\to$ 2s $\to$ 4s). Nếu API key gặp sự cố hoặc cạn kiệt request sau các lần thử, hệ thống chuyển đổi mượt mà sang công thức Haversine (tốc độ giả định $15\text{ km/h}$) để đảm bảo trải nghiệm người dùng không bị gián đoạn.

\subsubsection{Đánh giá và Tinh chỉnh Thuật toán}

Qua quá trình thử nghiệm, thuật toán cơ sở (Baseline) đã được phân tích và cải tiến (Refactor) đáng kể để khắc phục các hạn chế về logic và hiệu năng thực tế:

\begin{itemize}
  \item \textbf{Phân cụm không gian (K-Means++):} Thuật toán cũ khởi tạo tâm cụm (centroid) theo chỉ số mảng tuần tự, dẫn đến chất lượng phân cụm kém nếu dữ liệu từ Database không sắp xếp theo địa lý. Bản tinh chỉnh sử dụng thuật toán \textbf{K-Means++}, lựa chọn các tâm cụm theo xác suất khoảng cách để đảm bảo độ phân tán tối ưu.
  \item \textbf{Giải quyết xung đột (Gap Insertion):} Thay vì chỉ cố gắng chèn các địa điểm bị rớt lịch vào \textit{cuối} ngày kèm thời gian di chuyển gán cứng (10 phút) như bản cũ, hệ thống mới tính toán thời gian di chuyển thực và quét qua toàn bộ các \textit{khoảng trống} (gap) giữa các điểm đã lên lịch. Việc này giúp tận dụng tối đa quỹ thời gian rảnh của du khách.
  \item \textbf{Tối ưu luồng thời gian thực tế:} Thuật toán được bổ sung tham số cho phép cấu hình Giờ nghỉ trưa (Lunch Break, mặc định 11:00--13:00) và kiểm tra chặt chẽ \texttt{open\_days} ngay khi chọn điểm xuất phát đầu tiên, khắc phục lỗi thuật toán cũ đôi khi xếp lịch vào một điểm đang đóng cửa chỉ vì nó nằm gần nhất.
\end{itemize}

Để làm rõ hơn, Bảng~\ref{tab:algorithm_optimization} dưới đây tóm tắt các điểm khác biệt cốt lõi giữa hai phiên bản thuật toán:
...
\midrule
\textbf{Khởi tạo K-Means} & Chọn tâm cụm theo chỉ số mảng tuần tự. Gom cụm kém nếu dữ liệu không có thứ tự. & Áp dụng \textbf{K-Means++} kết hợp \textbf{Seeded PRNG} tạo tính xác định (Deterministic). \\ 
\addlinespace
\textbf{Giải quyết xung đột} & Chỉ nối điểm rớt lịch vào \textit{cuối ngày}. Gán cứng thời gian di chuyển (10 phút). & Áp dụng \textbf{Gap Insertion}. Dùng khoảng cách thực, quét mọi khoảng trống (gap) giữa các điểm để chèn. \\ 
\addlinespace
\textbf{Dự phòng Rate Limit} & Thử lại (retry) đúng 1 lần với thời gian chờ cố định 3 giây. & Áp dụng \textbf{Exponential Backoff} (3 lần thử: 1s $\to$ 2s $\to$ 4s), tăng độ ổn định của API. \\ 
\addlinespace
\textbf{Ràng buộc thời gian} & Bỏ qua giờ nghỉ trưa. Dễ xếp lịch nhầm vào điểm đang đóng cửa do chỉ ưu tiên khoảng cách. & Tích hợp \textbf{Lunch Break} vào cả luồng GNN và Cascade Insertion. Bắt buộc kiểm tra \texttt{open\_days} cho điểm đầu tiên, fix lỗi \texttt{firstWait}. \\ 
\addlinespace
\textbf{Khởi tạo điểm xuất phát} & Tính khoảng cách điểm đầu tiên bằng công thức thuần Haversine. & Nâng cấp dùng \textbf{Goong API} để tìm điểm xuất phát thực tế, có Haversine dự phòng (fallback). \\
\bottomrule
\end{tabular}
\end{table}
