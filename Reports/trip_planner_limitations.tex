...
\end{equation}
The visit starts at $V_j = \max(A'_j, O_j)$ and ends at $D_j = V_j + d_j$. The wait time (in seconds) is defined as $t_{\text{wait}}(j) = (V_j - A'_j) \times 60$. The transition is feasible if and only if the departure time satisfies the closing constraint:
\begin{equation}
...
\begin{equation}
\text{Cost}(p_{\text{curr}} \to p_j) = t(\text{curr}, j) \times 2 + t_{\text{wait}}(j)
\end{equation}
