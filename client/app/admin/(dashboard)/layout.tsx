...
export default function AdminLayout({
  ...
  const menuItems = [
    ...
    { name: 'Places', icon: 'account_balance', path: '/admin/places' },
    { name: 'Activities', icon: 'event', path: '/admin/activities' },
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto hide-scrollbar">
          ...
          {menuItems.map((item) => (
            <Link
              key={item.path}
              href={item.path}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all relative group ${pathname === item.path
